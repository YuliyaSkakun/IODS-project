...
scaled_crim <- scale(boston_scaled)
...
*LDA will fid a combination of the explanatory variable in such way so that it can separate the classes of the crime variable the best*
...
*The graph shows that biggest impact the radical hihways (blue) and high crime rates.*
...
* When performing k-means model, I have set the number of the clusters equal to 10. Howevr, from the plot (Within Cluster Sum of Squares), it obvious that 2 cluster will enough due to the fact that after two clusters, the value drops significantly. So, I will adjust the model.* 
...
*Also, here is another 3D plot where the color is defined by the clusters of the k-mean. It shos the same classes as in LDA model.*
